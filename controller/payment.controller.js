...
exports.paymentVerificationHandler = async (req, res) => {
    ...
    try {
        ...
        if (!isAuthentic) {
            const url = `${FRONTEND_URL}/paymentFailed?reason=Invalid_signature`
            return res.redirect(
                url
            );
        }
        ...
        if (!tempOrder) {
            const url = `${FRONTEND_URL}/paymentFailed?reason=Order_not_found`
            return res.redirect(
                url
            );
        }
        ...
        {
            const url = `${FRONTEND_URL}/paymentSuccess?reference=${razorpay_payment_id}`
            return res.redirect(
                url
            );
        }
